...
from utils.item_utils import ItemStatus, ItemType, SharePermission, verify_access
...
async def _get_item_model_or_404(item_id: str) -> ItemModel:
...
async def complete_item_upload(
    ...
    item = await _get_item_model_or_404(item_id)
...
async def get_folder_service(folder_id: str, current_user_id: str):
    folder_model = await _get_item_model_or_404(folder_id)
